        ...
        for entry in payload.get("entry", []):
            for change in entry.get("changes", []):
                ...
                for msg in value.get("messages", []):
                    ...
                    user = db.query(User).filter(User.whatsapp_phone_id == phone_number_id).order_by(User.id.asc()).first()
                    if not user:
...
async def _process_interactive_followup(db: Session, user: User, phone: str, reply_id: str):
    ...
    for menu in menus:
        ...
        if reply_id in rules:
            ...
            caption_legacy = (rule.get("caption") or "").strip()
            text_body = content or caption_legacy

            try:
                # ── TEXT ONLY ────────────────────────────────────────────
                if rule_type == "text":
                    if not text_body:
                        logger.warning(f"Follow-up text rule has empty content: {rule}")
                        return
                    result = await send_whatsapp_message(
                        user.whatsapp_phone_id, user.whatsapp_token,
                        phone, text_body
                ...
                # ── MEDIA (+ optional text) ──────────────────────────────
                    ...
                    raw_url = (rule.get("media_url") or "").strip()
                    # Legacy/relative URL → full public URL bana do
                    if raw_url and not raw_url.startswith(("http://", "https://")):
                        base    = (settings.BASE_URL or "").rstrip("/")
                        raw_url = f"{base}{raw_url if raw_url.startswith('/') else '/' + raw_url}"

                    if not raw_url:
                        logger.error(f"Follow-up media rule has empty URL: {rule}")
                        return

                    media_type = rule.get("media_type") or "image"
                    # WhatsApp image/video/document support inline caption,
                    # audio support nahi karta — us ke liye text alag bhejna hoga
                    supports_caption = media_type in ("image", "video", "document")
                    inline_caption   = text_body if supports_caption else ""

                    media_result = await send_whatsapp_media(
                        user.whatsapp_phone_id, user.whatsapp_token,
                        phone, media_type, raw_url, inline_caption
                    )
                    db.add(InboxMessage(
                        user_id=user.id, customer_phone=phone,
                        direction=MessageDirection.outbound,
                        message_type=media_type,
                        content=inline_caption,
                        media_url=raw_url,
                        whatsapp_message_id=media_result.get("message_id", ""),
                        whatsapp_status="sent",
                    ))
                    db.commit()

                    # Audio + text case: text alag message ke taur pe bhejo
                    if text_body and not supports_caption:
                        text_result = await send_whatsapp_message(
                            user.whatsapp_phone_id, user.whatsapp_token,
                            phone, text_body
                        )
                        db.add(InboxMessage(
                    ...
                    logger.warning(f"Unknown follow-up rule type: {rule_type}")
                    return

                logger.info(f"✅ Follow-up sent for reply_id: {reply_id} (type={rule_type})")
